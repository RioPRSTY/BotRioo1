...
prefix = "!"
name = "~ R I O P R S T Y"
rdaftar = "TERIMA KASIH TELAH DAFTAR MENJADI TEMEN RIOPRSTY😁"
rmenu = "HAI, 👋 JANGAN LUPA DONASI YAA:)"
limitt = 10
ban = []
...
async function starts() {
	...
	fs.existsSync('./anker.json') && client.loadAuthInfo('./anker.json')
	...
	await client.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./anker.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FarhanGans = ["081393060533@s.whatsapp.net"] // ubah aja gapapa
...
//akhir kreator
		    ...
		    case 'map':
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat DARKBOT\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *DARKBOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Tidak Terdaftar Di Dalam Database *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[DARKBOT]','aqua'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
